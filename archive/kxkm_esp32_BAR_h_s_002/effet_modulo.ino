...
void eff_modulo() {
  ...
  if ( type_modulo == 0 ) {
    ...
    for (int i = 0; i < n_led_to_show ; i ++) {
      ...
      strands[n_strip]->pixels[a - (NUM_LEDS_PER_STRIP * n_strip)] = pixelFromRGBW(r, g, b, w);
    }//for i
  }//modulo 3
    ...
    for (int i = 0; i < numberOfLed; i++) {
      if (randArray[i]) {
        ...
        strands[n_strip]->pixels[i - (NUM_LEDS_PER_STRIP * n_strip)] = pixelFromRGBW(r, g, b, w);
      }
    }
    return;
  }//modulo 6
