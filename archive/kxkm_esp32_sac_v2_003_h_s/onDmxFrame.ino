...
void onDmxFrame(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t* data) {
  ...
  mod = data[adr + 4];
  pix_mod = data[adr + 5];
  pix_start = data[adr + 6] - 1;

  pix_end = pix_start + pix_start;
  if (pix_mod >= 11 && pix_mod <= 20 || pix_mod >= 31 && pix_mod <= 60 ) {
    pix_pos = (((pix_start + NUM_LEDS_PER_STRIP + pix_end) * data[adr + 7]) / 255) - (pix_end + 1);
  } else if (pix_mod >= 21 && pix_mod <= 30 || pix_mod >= 61 && pix_mod <= 90) {
    pix_pos = (((pix_start + NUM_LEDS_PER_STRIP + pix_end) * data[adr + 7]) / 255) - (pix_end + 1);
  }
  modulo = data[adr + 8];
  str_ws = (data[adr + 9] * data[adr + 9]) / 33;
  pix_center = ((pix_start) / 2) + pix_pos;
  M_g = (NUM_LEDS_PER_STRIP + 1) / NUM_STRIPS;

  smaster = data[adr + 10];
  srr = (data[adr + 11] * data[adr + 11]) / 255;
  sgg = (data[adr + 12] * data[adr + 12]) / 255;
  sbb = (data[adr + 13] * data[adr + 13]) / 255;
  sww = (data[adr + 14] * data[adr + 14]) / 255;
